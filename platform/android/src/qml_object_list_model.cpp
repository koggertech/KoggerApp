...
KOGGER_LOGGING_CATEGORY(QmlObjectListModelLog, "API.QmlObjectListModel")
...
void QmlObjectListModel::setDirty(bool dirty)
{
    if (_dirty != dirty) {
        ...
        if (!dirty) {
            ...
            for (auto it = _objectList.cbegin(); it != _objectList.cend(); ++it) {
                if ((*it)->property("dirty").isValid()) {
                    (*it)->setProperty("dirty", false);
                }
            }
        }
