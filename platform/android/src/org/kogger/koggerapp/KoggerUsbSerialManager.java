...
public class KoggerUsbSerialManager {
    private static final String TAG = KoggerUsbSerialManager.class.getSimpleName();
    ...
    /**
     * Initializes the UsbSerialManager. Should be called once, typically from KoggerActivity.onCreate().
     *
    ...
    public static void initialize(Context context) {
        ...
        if (usbManager == null) {
            KoggerLogger.e(TAG, "Failed to get UsbManager");
            return;
        }
     ...
     * Should be called when the manager is no longer needed, typically from KoggerActivity.onDestroy().
     */
    public static void cleanup(Context context) {
        try {
            context.unregisterReceiver(usbReceiver);
            KoggerLogger.i(TAG, "BroadcastReceiver unregistered successfully.");
        } catch (final IllegalArgumentException e) {
            KoggerLogger.w(TAG, "Receiver not registered: " + e.getMessage());
        }
    }
    ...
    private static void registerUsbReceiver(Context context) {
        ...
        // TODO: Move bluetooth handling back to KoggerActivity
        ...
        try {
            ...
            KoggerLogger.i(TAG, "BroadcastReceiver registered successfully.");
        } catch (Exception e) {
            KoggerLogger.e(TAG, "Failed to register BroadcastReceiver", e);
        }
    }
    ...
    private static final BroadcastReceiver usbReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            KoggerLogger.i(TAG, "BroadcastReceiver USB action " + action);
    ...
    private static void handleUsbPermission(final Intent intent) {
        ...
        if (device != null) {
            ...
            if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
                KoggerLogger.i(TAG, "Permission granted to " + device.getDeviceName());
                addOrUpdateDevice(device);
            } else {
                KoggerLogger.i(TAG, "Permission denied for " + device.getDeviceName());
    ...
    public static int getDeviceId(final String deviceName) {
        ...
        if (driver == null) {
            KoggerLogger.w(TAG, "Attempt to get ID of unknown device " + deviceName);
    ...
    private static void addDriver(final UsbSerialDriver newDriver) {
        ...
        if (usbManager.hasPermission(device)) {
            ...
            KoggerLogger.i(TAG, "Requesting permission to use device " + deviceName);
            usbManager.requestPermission(device, usbPermissionIntent);
        }
    }
    ...
    private static UsbSerialPort findPortByDeviceId(final int deviceId) {
        ...
        if (deviceId == BAD_DEVICE_ID) {
            KoggerLogger.w(TAG, "Finding port failed for invalid Device ID " + deviceId);
            return null;
        }

        UsbSerialDriver driver = findDriverByDeviceId(deviceId);
        if (driver == null) {
            KoggerLogger.w(TAG, "No driver found on device ID " + deviceId);
            return null;
        }

        List<UsbSerialPort> ports = driver.getPorts();
        if (ports.isEmpty()) {
            KoggerLogger.w(TAG, "No ports available on device ID " + deviceId);
            return null;
        }

        if (portIndex < 0 || portIndex >= ports.size()) {
            KoggerLogger.w(TAG, "Invalid port index " + portIndex + " for device ID " + deviceId);
            return null;
        }
    ...
    public static int open(final String deviceName, final long classPtr) {
        UsbSerialDriver driver = findDriverByDeviceName(deviceName);
        if (driver == null) {
            KoggerLogger.w(TAG, "Attempt to open unknown device " + deviceName);
            return BAD_DEVICE_ID;
        }

        List<UsbSerialPort> ports = driver.getPorts();
        if (ports.isEmpty()) {
            KoggerLogger.w(TAG, "No ports available on device " + deviceName);
            return BAD_DEVICE_ID;
        }
        ...
        if (!openDriver(port, device, deviceId, classPtr)) {
            KoggerLogger.e(TAG, "Failed to open driver for device " + deviceName);
    ...
    private static boolean openDriver(final UsbSerialPort port, final UsbDevice device, final int deviceId, final long classPtr) {
        ...
        if (connection == null) {
            KoggerLogger.w(TAG, "No Usb Device Connection");
        ...
        try {
            ...
            KoggerLogger.e(TAG, "Error opening driver for device " + device.getDeviceName(), ex);
        ...
        KoggerLogger.d(TAG, "Port Driver open successful");
        return true;
    }
    ...
    private static boolean createIoManager(final int deviceId, final UsbSerialPort port, final long classPtr) {
        ...
        KoggerSerialListener listener = new KoggerSerialListener(classPtr);
        ...
        KoggerLogger.d(TAG, "Read Buffer Size: " + ioManager.getReadBufferSize());
        KoggerLogger.d(TAG, "Write Buffer Size: " + ioManager.getWriteBufferSize());
        ...
        try {
            ...
            KoggerLogger.e(TAG, "IO Manager configuration error:", e);
            return false;
        }
    ...
    public static boolean startIoManager(final int deviceId) {
        ...
        if (resources == null || resources.ioManager == null) {
            KoggerLogger.w(TAG, "IO Manager not found for device ID " + deviceId);
            return false;
        }
        ...
        try {
            ...
            KoggerLogger.d(TAG, "Serial I/O Manager started for device ID " + deviceId);
            return true;
        } catch (final IllegalStateException e) {
            KoggerLogger.e(TAG, "IO Manager Start exception:", e);
            return false;
        }
    }
    ...
    public static boolean stopIoManager(final int deviceId) {
        ...
        KoggerLogger.d(TAG, "Serial I/O Manager stopped for device ID " + deviceId);
        return true;
    }
    ...
    public static boolean close(int deviceId) {
        ...
        try {
            ...
            KoggerLogger.e(TAG, "Error closing driver:", ex);
            return false;
        }
    }
    ...
    public static int write(final int deviceId, final byte[] data, final int length, final int timeoutMSec) {
        UsbSerialPort port = findPortByDeviceId(deviceId);
        if (port == null) {
            KoggerLogger.w(TAG, "Attempted to write to a null port for device ID " + deviceId);
            return -1;
        }

        if (!port.isOpen()) {
            KoggerLogger.w(TAG, "Attempted to write to a closed port for device ID " + deviceId);
            return -1;
        }
        ...
        try {
            ...
            KoggerLogger.e(TAG, "Write timeout occurred", e);
            return -1;
        } catch (final IOException e) {
            KoggerLogger.e(TAG, "Error writing data", e);
            return -1;
        }
    }
    ...
    public static int writeAsync(final int deviceId, final byte[] data, final int timeoutMSec) {
        ...
        if (resources.ioManager.getReadTimeout() == 0) {
            KoggerLogger.w(TAG, "Read Timeout is 0 for writeAsync");
        }
    ...
    public static byte[] read(final int deviceId, final int length, final int timeoutMs) {
        if (timeoutMs < 500) {
            KoggerLogger.w(TAG, "Read with timeout less than recommended minimum of 200-500ms");
        }

        UsbSerialPort port = findPortByDeviceId(deviceId);
        if (port == null) {
            KoggerLogger.w(TAG, "Attempted to read from a null port for device ID " + deviceId);
            return new byte[]{};
        }

        if (!port.isOpen()) {
            KoggerLogger.w(TAG, "Attempted to read from a closed port for device ID " + deviceId);
            return new byte[]{};
        }
        ...
        try {
            ...
            KoggerLogger.e(TAG, "Error reading data", e);
        }
    ...
    public static boolean setParameters(final int deviceId, final int baudRate, final int dataBits, final int stopBits, final int parity) {
        UsbSerialPort port = findPortByDeviceId(deviceId);
        if (port == null) {
            KoggerLogger.w(TAG, "Attempted to set parameters to a null port for device ID " + deviceId);
            return false;
        }

        if (!port.isOpen()) {
            KoggerLogger.w(TAG, "Attempted to set parameters on a closed port for device ID " + deviceId);
            return false;
        }
        ...
        try {
            ...
            KoggerLogger.w(TAG, "Error setting parameters" + ": " + e);
            return false;
        } catch (final IOException e) {
            KoggerLogger.e(TAG, "Error setting parameters", e);
            return false;
        }
    ...
    private static boolean getControlLine(int deviceId, UsbSerialPort.ControlLine controlLine) {
        UsbSerialPort port = findPortByDeviceId(deviceId);
        if (port == null) {
            KoggerLogger.w(TAG, "Attempted to get " + controlLine + " from a null port for device ID " + deviceId);
            return false;
        }

        if (!isControlLineSupported(port, controlLine)) {
            KoggerLogger.w(TAG, "Getting " + controlLine + " Not Supported");
            return false;
        }
        ...
        try {
            ...
            KoggerLogger.w(TAG, "Error getting " + controlLine + ": " + e);
            return false;
        }  catch (final IOException e) {
            KoggerLogger.e(TAG, "Error getting " + controlLine, e);
            return false;
        }
    }
    ...
    private static boolean setControlLine(int deviceId, UsbSerialPort.ControlLine controlLine, boolean on) {
        UsbSerialPort port = findPortByDeviceId(deviceId);
        if (port == null) {
            KoggerLogger.w(TAG, "Attempted to set " + controlLine + " on a null port for device ID " + deviceId);
            return false;
        }

        if (!isControlLineSupported(port, controlLine)) {
            KoggerLogger.e(TAG, "Setting " + controlLine + " Not Supported");
            return false;
        }
        ...
        try {
            switch (controlLine) {
                ...
                case RTS:
                    ...
                    break;
                default:
                    KoggerLogger.w(TAG, "Setting " + controlLine + " is not supported via this method.");
                    return false;
            }
        } catch (final UnsupportedOperationException e) {
            KoggerLogger.w(TAG, "Error setting " + controlLine + ": " + e);
            return false;
        } catch (final IOException e) {
            KoggerLogger.e(TAG, "Error setting " + controlLine, e);
            return false;
        }
    ...
    private static boolean isControlLineSupported(final UsbSerialPort port, final UsbSerialPort.ControlLine controlLine) {
        ...
        try {
            ...
            KoggerLogger.e(TAG, "Error getting supported control lines", e);
            return false;
        }
    ...
    public static int[] getControlLines(final int deviceId) {
        ...
        if (port == null || !port.isOpen()) {
            KoggerLogger.w(TAG, "Attempted to get control lines from a null or closed port for device ID " + deviceId);
            return new int[]{};
        }
        ...
        try {
            ...
            KoggerLogger.w(TAG, "Error getting control lines: " + e);
            return new int[]{};
        } catch (final IOException e) {
            KoggerLogger.e(TAG, "Error getting control lines", e);
            return new int[]{};
        }
    ...
    public static int getFlowControl(final int deviceId) {
        UsbSerialPort port = findPortByDeviceId(deviceId);
        if (port == null || !port.isOpen()) {
            KoggerLogger.w(TAG, "Attempted to get flow control from a null or closed port for device ID " + deviceId);
            return 0;
        }

        EnumSet<UsbSerialPort.FlowControl> supportedFlowControl = port.getSupportedFlowControl();
        if (supportedFlowControl.isEmpty()) {
            KoggerLogger.e(TAG, "Flow Control Not Supported");
            return 0;
        }
    ...
    public static boolean setFlowControl(final int deviceId, final int flowControl) {
        ...
        if (flowControl < 0 || flowControl >= UsbSerialPort.FlowControl.values().length) {
            KoggerLogger.w(TAG, "Invalid flow control ordinal " + flowControl);
            return false;
        }
        ...
        if (port == null || !port.isOpen()) {
            KoggerLogger.w(TAG, "Attempted to set flow control on a null or closed port for device ID " + deviceId);
            return false;
        }

        EnumSet<UsbSerialPort.FlowControl> supportedFlowControl = port.getSupportedFlowControl();
        if (!supportedFlowControl.contains(flowControlEnum)) {
            KoggerLogger.e(TAG, "Setting Flow Control Not Supported");
            return false;
        }
        ...
        try {
            ...
            KoggerLogger.w(TAG, "Error setting Flow Control: " + e);
            return false;
        } catch (final IOException e) {
            KoggerLogger.e(TAG, "Error setting Flow Control", e);
            return false;
        }
    ...
    public static boolean setBreak(final int deviceId, final boolean on) {
        UsbSerialPort port = findPortByDeviceId(deviceId);
        if (port == null || !port.isOpen()) {
            KoggerLogger.w(TAG, "Attempted to set break on a null or closed port for device ID " + deviceId);
            return false;
        }
        ...
        try {
            ...
            KoggerLogger.w(TAG, "Error setting break condition: " + e);
            return false;
        } catch (final IOException e) {
            KoggerLogger.e(TAG, "Error setting break condition", e);
            return false;
        }
    ...
    public static boolean purgeBuffers(final int deviceId, final boolean input, final boolean output) {
        UsbSerialPort port = findPortByDeviceId(deviceId);
        if (port == null || !port.isOpen()) {
            KoggerLogger.w(TAG, "Attempted to purge buffers on a null or closed port for device ID " + deviceId);
            return false;
        }
        ...
        try {
            ...
            KoggerLogger.w(TAG, "Error purging buffers: " + e);
            return false;
        } catch (final IOException e) {
            KoggerLogger.e(TAG, "Error purging buffers", e);
            return false;
        }
